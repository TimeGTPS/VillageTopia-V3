...
inline void saveAllWorlds() // 
{
	...
	Sleep(200);
	GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`w[`9GrowCastle`w] `2Saved `oall worlds``"));
...
inline void DailyRewardCheck(ENetPeer* peer)
{
	...
	if (static_cast<PlayerInfo*>(peer->data)->startkit == false)
	{
		...
		SaveItemMoreTimes(242, 50, peer, success);
		SaveItemMoreTimes(1796, 25, peer, success);
		SaveItemMoreTimes(98, 1, peer, success);
		Player::onGiveGems(peer, 1000000);
		static_cast<PlayerInfo*>(peer->data)->startkit = true;
		Player::OnConsoleMessage(peer, "`^You got`2 1000000 `9Gems `^And `2Start Pack `^for registering!");
	}
}
...
inline void autoSaveWorlds() {
	while (1) {
		...
		GamePacket p222 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`w[`4SYSTEM`w] `5Saving Worlds in a minute."));
		...
		GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`w[`9GrowCastle`w] `5Auto Saving All Worlds..."));
		ENetPacket* packet = enet_packet_create(p.data,
			p.len,
			ENET_PACKET_FLAG_RELIABLE);
		enet_host_broadcast(server, 0, packet);
