...
inline void banLoginDevice(ENetPeer* peer, const long long banDurationDefault, string sid, string mac)
{
	...
	const string dc = "https://discord.gg/fj6F3U6qTA";
	const auto url = "action|set_url\nurl|" + dc + "\nlabel|Join GrowCastle Discord\n";
...
inline void checkTimeBan(ENetPeer* peer, string username)
{
	...
	if (exist)
	{
		...
		if (fd.fail())
		{
			...
			Player::OnConsoleMessage(peer, "`5[Stream-ERROR]: `@ failed to check whether '" + username + "' is time banned. `5Please send this screenshot to GrowCastle developers.");
		...
		string dc = "https://discord.gg/fj6F3U6qTA";
...
inline void GiveRank(string rankName, string playerName, bool& success)
{
		...
		//else if (PlayerDB::getProperName(rankName) == "coming soon")
		//{
			//rankLevel = 199;
		//}
...
inline void LoadPlayerData(ENetPeer* peer)
{
	if (static_cast<PlayerInfo*>(peer->data)->haveGrowId)
	{
		...
		case 6:
			...
			break;
		case 5:
			static_cast<PlayerInfo*>(peer->data)->displayName = "`w[`eManager`w]" + static_cast<PlayerInfo*>(peer->data)->tankIDName;
			static_cast<PlayerInfo*>(peer->data)->displayNamebackup = "`w[`eManager`w]" + static_cast<PlayerInfo*>(peer->data)->tankIDName;
			break;
		case 4:
